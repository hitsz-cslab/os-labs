...
| int mknod(char *name, int major, int minor)         | Create a device file. |
...
筛选思路：主进程将所有数据（例2~11）输入到管道的左侧，第一个子进程从管道读出并筛选出2，排除掉2的倍数，其他数字再写入下一管道；第二个子进程读出并筛选出3，排除掉3的倍数，其他数字写入到下一管道；第三个子进程读出筛选出5，以此类推……如下图所示：

![image-20201017231043674](part2.assets/image-20201017231043674.png)

可参考[Bell Labs and CSP Threads](https://swtch.com/~rsc/thread/)
