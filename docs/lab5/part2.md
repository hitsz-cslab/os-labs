...
在文件系统中，任何一个文件都是由元数据（inode）和数据（data）构成。文件夹（目录）也属于这里的文件范畴，目录也是一种文件。元数据存放在索引节点区，数据存放在数据块取，并通过元数据来索引数据块。
...
通过元数据inode索引到数据块后，就可以存放和读取文件的数据。

**（2）数据（data）通常存放：**

- **普通文件的话**，直接存放文件的内容即可。例如，file.txt的内容为aaa，那么file.txt的数据块直接填aaa即可。
- **目录文件的话**，存放所有的子目录项（dentry）。例如，目录dir下有子目录dir0和子文件file.txt，那么目录dir的数据块填的是子目录dir0的dentry结构和子文件file.txt的dentry结构。
...
上面讲述的元数据（inode）和数据（data）表示，只是给出了一个文件自身的组织形式。但是，文件和文件之间，文件和目录之间的关系如何表示呢？例如，dir0是dir的子目录，file又是dir0的子文件。仅仅有上述的结构无法表示**文件的层级关系**。
