...
![ext2布局](part2.assets/ext2布局.png)
...
- **根目录索引**。最上层的第一个目录就是根目录，是所有目录解析的入口，根目录维护在超级块结构中供全局访问。
...
struct super_block_d 
{
    ...
    /* 根目录索引 */
    int	root_ino;			// 根目录对应的inode，通常默认第0个
...
在文件系统中，任何一个文件都是由 **索引节点（inode）** 和 **数据（data）** 构成。索引节点（inode）的数据存放在索引节点区，数据（data）存放在数据块区，并通过索引节点（inode）来索引数据块。

文件夹（目录）也属于这里的文件范畴，目录也是一种文件。没有特殊说明，本小节的文件包括普通文件（file）和目录文件（dir）。

简单的示意图，如下图所示：

![文件组织形式](part2.assets/文件组织形式.png)
...
以一个普通文件为例，下面的图展示了一个文件的元数据和数据如何分离，并如何如何通过元数据来索引数据。

![image-20211024102812913](part2.assets/image-20211024102812913.png)

#### 1.2.4 目录项的表示

上面讲述的索引节点（inode）和数据（data）表示，只是给出了一个文件 **自身** 的组织形式。但是，目录文件（dir）及其子文件之间的关系如何表示呢？例如，dir0是dir的子目录，file又是dir0的子文件。仅仅靠上一小节的索引节点（inode）结构无法表示 **文件的层级关系** 。

为此，类似于每个文件都有一个索引节点（inode），我们额外为每一个文件都维护一个 **很精小的**（比索引节点inode还小） 的标识的结构，这个标识可以找到文件的索引节点（inode）。如下图所示：

![目录项](part2.assets/目录项.png)

每个文件都有了一个标识。目录文件只需要把所有子文件的标识，都存放在自己的数据块里。通过读取目录文件的数据块，逐个读取出标识，就能够知道这个目录下有哪些子文件了。这个标识就是 **目录项（dentry）**。每一个文件都会有一个目录项（dentry）。例如，体现`dir`目录下有`file1`、`dir0`、`file2`的一个图示结构，如下所示：

![目录项](part2.assets/dentry示例.png)

一个目录项要尽可能精简，只需要包含文件最基本的信息，通常会包括：
...
当文件系统挂载起来后，运行于内存中。当文件系统卸载后，所有信息 **刷回** 磁盘中。下一次挂载重新挂载文件系统的时候，会 **读取** 之前刷回磁盘的内容，**重新构建** 文件系统的有关数据结构，例如：super_block（超级块）、inode（索引节点）、dentry（目录项）、inode位图等。简单的图示如下：

![虚拟磁盘](part2.assets/文件系统一致性.png)
...
![虚拟磁盘](part2.assets/虚拟磁盘.png)



本次实验在用户态下模拟了一个 **容量为4MB的磁盘** ，并实现了对这个虚拟磁盘进行操作的 **DDRIVER驱动** 。 **虚拟磁盘** 是通过一个普通的数据文件来充当，这个数据文件的路径位于 `~/disk` 。我们封装了一层DDRIVER驱动，其原理是单次读取、写入这个数据文件时会按照固定的大小 `512B` 进行读取和写入，也就是 **IO大小为512B** ，从而来模拟达到磁盘操作。
...
DDRIVER驱动封装实现了open，close，seek，read，write，ioctl的方法，同学在文件系统实现中直接调用这些驱动接口来和 **磁盘进行读写交互** 即可，无需关注其具体实现原理。

| DDRIVER驱动API                                          | 接口介绍                                                     |
| ------------------------------------------------------- | ------------------------------------------------------------ |
| int ddriver_open(char *path)                            | 根据路径path打开虚拟磁盘，返回文件描述符fd。path只能固定传入`~/disk`的绝对路径。 |
...
!!! note "补充"
    这部分的相关代码位于`user-land-filesystem/driver/user_ddriver`下，有兴趣的同学可以简单阅读。实验没有涉及 `user-land-filesystem/driver/kernel_ddriver` 部分。最终模拟驱动最终编译为了静态链接库，是为了无法直接通过Debug调试进入到上述接口实现的代码中， 出自模拟真实驱动所做的考虑，了解即可。
...
关于查看虚拟磁盘的内容，同学需要在VSCode上安装Hex Editor插件，然后右键点击ddriver_dump，选择打开方式，选择Hex Editor，就可以查看自己虚拟磁盘的内容了，调试可能会用到。
...
![FUSE文件系统](part2.assets/FUSE文件系统.png)
