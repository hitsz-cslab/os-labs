...
如下图所示，所有的路径解析都会从 **根目录** 开始，由超级块维护的根目录`dentry`，读取根目录的`inode`（在读取`inode`的时候会把数据块的内容读取进来，即所有子文件的`dentry_d`），然后依次遍历所有子文件的`dentry`，找到文件名为`home`的`dentry`，然后再读取`home`的inode，重复上面步骤。解析到 **倒数第二层** 的`dentry`，就代表解析成功，返回该目录的`dentry`。为了判断解析到第几层了，可以对路径进行预处理，通过判断路径中`/`的个数来知道总层数`total_level`是多少，然后解析的过程维护一个当前层级`cur_level`。



![](./part3.assets/lookup示意图.png)
