...
但是，xv6使用了共享的内核页表，所有进程使用一个页表，这就意味着内核进行 **进程间调度** 的时候，不需要切换页表。在进程调度器`schedular`函数当中，其流程体现出这一特点：  

![schedular-pgtbl-before](part2.assets/schedular-pgtbl-before.png)

可以看见：由于使用 **内核全局页表** ，在进行进程间调度时，**不需要对`satp`寄存器进行写入操作** ，只需要进行进程上下文的切换即可。

但是如果内核页表变成独立页表，那么在进程调度，切换进程的时候，就需要 **在适当的时候切换内核页表** （我们会在[实验实现](../part3/#21)当中给出具体的图片介绍）。
