...
1. 将数据结构改为 **数组**，每个 buf 拥有自己的锁（类似一个每个桶 1 个 buf 的哈希桶）。

2. `bget()` 查找时，不需要获取对应的锁，查找到时，锁上对应的锁，然后重新检查这个 buf 是否是想要的，检查完后进行对应的操作。

3. 每个 buf 维护一个 Access Bit，当 `bget()` 没有找到对应的缓存 buf 时，根据 CLOCK 算法维护 Access Bit 并分配一个 buf。

CLOCK 算法实现思路中我们提到，查找时不需要获取对应的锁，那么这个是否安全？如果查找时也获取对应的锁，会发生什么？感兴趣的同学们可以思考，也可以在搜索引擎搜索关键词”乐观锁“和”悲观锁“

上述方案在以下情况中都存在少量锁争用，在本实验中，这是允许的：
...
bcache 与前面 kalloc 实验的重要区别是，bcache 实验需要处理分配/替换情况，因此同学们第一次设计 **极易** 出现死锁。
...
!!! warning "注意"
      即使你的设计通过了全部的测试，也不代表你的代码没有死锁。
      设计方案时形式化地思考理论课上学习的知识，有助于你设计出正确的代码。
...
下面给出一种替换/分配思路，这个思路也出现了同时持有两个或以上自旋锁的情况，请同学们思考这种做法是否有死锁，是否出现竞争。

a) 首先获取请求对应桶的锁，查找不到以后进入分配/替换流程，释放锁。

b) 然后获取全局分配大锁，获取原有桶的锁，获取新桶的锁，进行替换逻辑，按序释放所有锁。
